...
class ResearchAgent(BaseAgent):
    def __init__(self, name: str, model_name: str, max_result: int):
        ...
        logger.info(f"[{self.name}] 🔧 Агент инициализирован с моделью {model_name}, max_result={max_result}")

    def searching(self, state: SearchWorkflowState) -> SearchWorkflowState:
        query = state["search_query"]
        logger.info(f"[{self.name}] 🔍 Начинаю поиск по запросу: '{query}' (макс. результатов: {self.max_result})")
        
        try:
            search_results = DDGS().text(query, max_results=self.max_result)  # type: ignore
            logger.info(f"[{self.name}] 📊 Получено {len(search_results)} результатов от поисковика")
        ...
        for idx, x in enumerate(search_results, 1):
            ...
            if url is None:
                logger.warning(f"[{self.name}] ⚠️ Результат #{idx}: URL отсутствует, пропускаю")
                continue
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    processed_count += 1
                    logger.success(f"[{self.name}] ✅ Успешно загружено: {url} ({len(r.text)} символов)")
                else:
        ...
        logger.info(f"[{self.name}] 📋 Итог поиска: {processed_count} успешно, {failed_count} с ошибками, всего {len(results)} источников")
        return state
    ...
    async def extract_text_from_search(self, state: RawDocument):
        ...
        try:
            ...
            logger.info(f"[{self.name}] ✅ Извлечено {extracted_length} символов за {processing_time:.2f} сек")
            logger.debug(f"[{self.name}] 📝 Результат извлечения (первые 500 символов): {response.content[:500]}...")
